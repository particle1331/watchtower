...
4. Stage only the files belonging to the current group and commit each group with a concise Conventional Commit message such as `feat(skills): add shared notebook skill setup` or `docs(content): improve weak supervision explanation`.
